...
const EVENT_WRITE_CUTOFF = new Date("2026-07-12T13:00:00Z");

function isEventClosed() {
  return Date.now() >= EVENT_WRITE_CUTOFF.getTime();
}
...
const copy = {
  en: {
    ...
    eventClosedTitle: "The event has closed",
    eventClosedBody: "Thank you for participating. Ratings and other information can no longer be updated.",
    eventClosedDone: "Done",
  ...
  es: {
    ...
    eventClosedTitle: "El evento ha finalizado",
    eventClosedBody: "Gracias por participar. Las calificaciones y demás información ya no se pueden actualizar.",
    eventClosedDone: "Listo",
...
const eventClosedDialog = document.getElementById("event-closed-dialog");
...
function updateEventClosedDialog() {
  const lang = copy[state.language];
  document.getElementById("event-closed-title").textContent = lang.eventClosedTitle;
  document.getElementById("event-closed-copy").textContent = lang.eventClosedBody;
  document.getElementById("event-closed-button").textContent = lang.eventClosedDone;
}

function showEventClosedNotice() {
  if (!isEventClosed() || eventClosedDialog.open) return;
  updateEventClosedDialog();
  eventClosedDialog.showModal();
}
...
function distributorCardClass(distributor) {
  const layout = ["cabarcas_sangria", "felipe_motta", "global_brands", "millennial", "tzanetatos", "vinos_garbor"].includes(distributor.id) ? "is-stacked" : "";
  return `${layout} distributor-${distributor.id}`.trim();
}
      ...
      <div class="wine-action-row">
        <div>
          ...
          <div class="rating-row">
            ${[1, 2, 3, 4, 5].map((value) => `
              <button class="${value <= rating ? "is-selected" : ""}" data-rate="${value}" aria-label="${value} ${lang.starsAria}" ${isEventClosed() ? "disabled" : ""}>★</button>
            `).join("")}
          </div>
        </div>
        <button class="small-button interest-inline ${saved ? "is-saved" : ""}" data-interest="${wineId}" ${(saved || isEventClosed()) ? "disabled" : ""}>${saved ? copy[state.language].onList : copy[state.language].interested}</button>
      </div>
...
document.addEventListener("click", (event) => {
  ...
  if (action === "close-event-closed") {
    eventClosedDialog.close();
    return;
  }
