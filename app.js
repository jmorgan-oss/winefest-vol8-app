...
function wineNotesWithAbv(wine) {
  const notes = (wine.notes || "").trim();
  if (!wine.alcoholPercent) return notes;
  if (!notes) return `ABV ${wine.alcoholPercent}`;
  const spacer = /[.!?]$/.test(notes) ? " " : ". ";
  return `${notes}${spacer}ABV ${wine.alcoholPercent}.`;
}
...
function distributorLogoClass(distributor) {
  return ["felipe_motta", "vinos_garbor"].includes(distributor.id) ? "is-wide" : "is-square";
}
...
function mapSupabaseData({ appTables, appTastings, dbDistributors, dbMenuItems, dbScheduleItems, dbEventTop10 }) {
  ...
  appTastings.forEach((item) => {
    nextWines[item.id] = {
      ...
      notes: item.notes || "",
      alcoholPercent: item.alcohol_percent || "",
      brand: item.brand || "",
      grape: item.grape || "",
      es: {
        type: item.beverage_type || "",
...
function renderTables() {
  ...
  container.innerHTML = tables.map((table) => `
    <button class="table-card" data-table="${table.id}">
      ...
      <span>
        <h3>${table.name}</h3>
        <span class="wine-meta">${tableCardMeta(table, lang)}</span>
      </span>
...
function renderWineDetail(wineId) {
  ...
  container.innerHTML = `
    <div class="detail-panel">
      <h2>${wine.name}</h2>
      <p class="wine-notes">${wineNotesWithAbv(wine)}</p>
      ...
      <div class="detail-panel">
        ...
        <div class="distributor-grid">
          ${Object.values(distributors).map((distributor) => {
            const primaryEmail = (distributor.contactEmail || "").split("/")[0].trim();
            return `
            <article class="distributor-card">
              ...
              <div class="distributor-copy">
                ...
                ${distributor.contactEmail ? `<a class="distributor-email" href="mailto:${primaryEmail}">${distributor.contactEmail}</a>` : ""}
              </div>
            </article>
          `}).join("")}
        </div>
