...
const copy = {
  en: {
    ...
    infoKicker: "Event Info",
    infoTitle: "Everything for today",
...
function mapSupabaseData({ appTables, appTastings, dbDistributors, dbMenuItems, dbScheduleItems, dbEventTop10 }) {
  ...
  const nextTables = appTables.map((item) => ({
    id: item.id,
    number: item.code,
    name: `${item.distributor_name} ${item.code}`,
...
function renderTables() {
  const lang = copy[state.language];
  document.querySelector("#view-taste .section-intro h2").textContent = lang.tasteTitle;
  document.querySelector("#view-taste .section-intro p").textContent = lang.tasteIntro;
  ...
  container.innerHTML = tables.map((table) => `
    <button class="table-card" data-table="${table.id}">
      ...
      <span>
        <h3>${table.name}</h3>
        <span class="wine-meta">${table.wines.length} ${lang.selections} · ${tableDistributorNames(table)}</span>
      </span>
...
function renderTableDetail(tableId) {
  ...
  container.innerHTML = `
    <div class="detail-panel">
      <div class="detail-meta">${lang.table} ${table.number} · ${lang.sampleData}</div>
      <h2>${table.name}</h2>
      <p>${tableDistributorNames(table)} ${lang.pouringText} ${table.wines.length} ${lang.featuredSelections}.</p>
...
function renderWineDetail(wineId) {
  ...
  container.innerHTML = `
    <div class="detail-panel">
      <div class="detail-meta">${lang.table} ${table?.number || ""} · ${distributor?.name || ""} · ${lang.sampleData}</div>
      <h2>${wine.name}</h2>
      <p>${wine.notes}</p>
      <div class="tag-row">
        <span class="tag">${wine.type}</span>
        <span class="tag">${wine.country}</span>
        <span class="tag">${wine.brand || wine.region || ""}</span>
      </div>
      <div class="stats-grid">
        <div class="stat-card"><span>${lang.typeLabel}</span><strong>${wine.type || "-"}</strong></div>
        <div class="stat-card"><span>${lang.grapeLabel}</span><strong>${wine.grape || wine.pairing || "-"}</strong></div>
...
function renderSubInfo(type) {
  ...
  const content = {
    ...
    distributors: `
      <div class="detail-panel">
        <div class="detail-meta">${lang.distributors} · ${lang.sampleData}</div>
        <h2>${lang.pouring}</h2>
        <div class="list-stack">
          ${Object.values(distributors).map((distributor) => {
            const tableLabels = distributor.tableIds.map((id) => tableNumber(id)).join(", ");
            const selectionCount = Object.values(wines).filter((wine) => wine.distributorId === distributor.id).length;
            return `
            <button class="pick-card" data-distributor="${distributor.id}">
              <div>
                <h4>${distributor.name}</h4>
                <div class="wine-meta">${lang.table} ${tableLabels} · ${selectionCount} ${lang.selections}</div>
              </div>
              <span class="chevron">›</span>
            </button>
          `}).join("")}
    ...
    schedule: `
      <div class="detail-panel">
        <div class="detail-meta">${lang.schedule} · ${lang.sampleData}</div>
    ...
    menu: `
      <div class="detail-panel">
        <div class="menu-heading">
          ...
          <div>
            <div class="detail-meta">${lang.foodBy} · ${lang.sampleData}</div>
            <p>${lang.menuLocation}</p>
          </div>
        </div>
        <h2>${lang.menu}</h2>
        <div class="list-stack">
          ${(menuItems.length ? menuItems : []).map((item) => `
            <article class="menu-item">
              <div>
                ...
                <div class="wine-meta">${item.pairing_note || ""}</div>
              </div>
...
function renderDistributor(distributorId) {
  ...
  const distributorWines = Object.entries(wines).filter(([, wine]) => wine.distributorId === resolvedDistributorId);
  const tableLabels = distributor.tableIds.map((id) => tableNumber(id)).join(", ");
  document.getElementById("distributor-detail").innerHTML = `
    <div class="detail-panel">
      <div class="detail-meta">${lang.table} ${tableLabels} · ${lang.sampleData}</div>
      <h2>${distributor.name}</h2>
      <p>${distributor.description}</p>
      <p class="muted-note">${lang.distributorNote}</p>
      <div class="tag-row">
        <span class="tag">${distributor.contact}</span>
      </div>
      <h3 class="section-title">${lang.selectionsTitle}</h3>
      <div class="list-stack">
        ${distributorWines.map(([id, wine]) => `
          <button class="wine-row" data-wine="${id}">
            <span>
              <h4>${wine.name}</h4>
              <span class="wine-meta">${localizedWine(wine).type} · ${localizedWine(wine).country}</span>
            </span>
            <span class="stars">${stars(state.ratings[id] || 0)}</span>
          </button>
        `).join("")}
...
function updateLanguage(lang) {
  ...
  document.querySelector("#view-taste .mini-label").textContent = selectedCopy.tasteKicker;
  ...
  document.querySelector("#brand-info-state h2").textContent = selectedCopy.brandInfoTitle;
...
document.addEventListener("click", (event) => {
  ...
  if (action === "brand-email") {
    if (hasSavedContact()) {
      saveBrandLeadFromGuest("begasa_contact_button_saved_guest");
      document.querySelector("#brand-success-state p").textContent = copy[state.language].brandDirectSavedBody;
      setBrandState("brand-success-state");
    } else {
      populateGuestForms();
      setBrandState("brand-lead-state");
  ...
  if (subview) {
    renderSubInfo(subview.dataset.subview);
    return;
  }
